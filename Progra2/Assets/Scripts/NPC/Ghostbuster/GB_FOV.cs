...
public class GB_FOV : MonoBehaviour
{
    ...
    private void FOVCheck()
    {
        ...
        {
            Player target = null;
            foreach(var posibleTarget in rangeChecks)
            {
                if (posibleTarget.transform.TryGetComponent<Player>(out Player newTarget))
                {
                    target = newTarget;
                }              
            }
            ...
            if (target !=null && Vector3.Angle(transform.forward, directionToTarget) < _angle / 2)
            {
