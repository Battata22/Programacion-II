...
public class GbChildDetector : MonoBehaviour
{
    [SerializeField] NPC _myOwner;
    [SerializeField] LayerMask obstructions;

    private void Start()
    {
        _myOwner = GetComponentInParent<NPC>();
    }

    private void OnTriggerEnter(Collider other)
    {
        if(other.gameObject.TryGetComponent<ChildScript>(out var child) && child.ChasingPlayer)
        {
